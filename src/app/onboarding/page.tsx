...
export const metadata = { title: 'Create workspace | ProcureIQ', description: 'Set up your ProcureIQ company workspace.' };
export default async function OnboardingPage() { const { user } = await requirePageUser(); const existing = await getPrimaryWorkspace(user.id); if (existing) redirect('/app/dashboard'); return <main><section className="auth-shell onboarding-shell"><div><p className="eyebrow">Workspace setup</p><h1>Create your company workspace.</h1><p>ProcureIQ scopes every RFQ, supplier, quote document, approval, and audit event to a workspace so company data stays separated and controlled.</p></div><OnboardingForm /></section></main>; }
