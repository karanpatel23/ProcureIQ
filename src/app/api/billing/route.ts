...
export async function POST(request: Request) { try { const { workspace } = await requireWorkspace(['owner', 'admin']); const input = await parseJson(request, billingPlanSchema); const updated = await mutateDb((db) => { const record = db.workspaces.find((item) => item.id === workspace.id); if (!record) return workspace; record.plan = input.plan; record.subscriptionStatus = env.BILLING_PROVIDER === 'stripe' && env.STRIPE_SECRET_KEY ? 'trialing' : 'not_configured'; record.updatedAt = now(); return record; }); return jsonOk({ workspace: updated, billingReady: env.BILLING_PROVIDER === 'stripe' && Boolean(env.STRIPE_SECRET_KEY) }); } catch (error) { return handleApiError(error); } }
