...
const NEXT_STATUS: Record<z.infer<typeof decisionSchema>['action'], WorkflowRunStatus> = {
  approve: 'approved',
  reject: 'rejected',
  edit: 'awaiting_approval', // an edit keeps it open for another human pass
  regenerate: 'awaiting_approval',
};

// The human checkpoint. A loop cannot finalize itself — only a person moves it
// out of awaiting_approval, and that decision is recorded for the audit trail.
export async function POST(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
    ...
    await writeAuditLog({
      workspaceId: workspace.id,
      actorUserId: user.id,
      action: `rfq_loop.${input.action}`,
      entityType: 'workflow_run',
      entityId: run.id,
      metadata: { status: run.status, notes: input.notes ?? '' },
    });

    return jsonOk({ runId: run.id, status: run.status, decision: run.decision });
  } catch (error) {
