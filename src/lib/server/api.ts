...
import { createRequestId, logServerError } from './observability';
...
export function jsonOk<T>(data: T, init?: ResponseInit) { return NextResponse.json<ApiSuccess<T>>({ ok: true, data, requestId: createRequestId() }, init); }
export function jsonError(error: ApiError) { return NextResponse.json<ApiFailure>({ ok: false, error: { code: error.code, message: error.message, details: error.details, requestId: createRequestId() } }, { status: error.status }); }
