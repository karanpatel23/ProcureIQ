...
const envSchema = z.object({
  ...
  const isStaticBuild = process.env.NEXT_PHASE === 'phase-production-build' || process.env.npm_lifecycle_event === 'build';
  if (value.NODE_ENV === 'production' && !isStaticBuild && value.AUTH_SECRET === 'development-auth-secret-change-before-production') ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['AUTH_SECRET'], message: 'Set a unique AUTH_SECRET before production deploys.' });
  if (value.NODE_ENV === 'production' && !isStaticBuild && !value.DATABASE_URL) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['DATABASE_URL'], message: 'DATABASE_URL is required for production deploys.' });
